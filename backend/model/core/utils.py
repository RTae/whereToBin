...
from model.core.config import cfg
...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if show_label:
            ...
            c3 = (c1[0] + t_size[0], c1[1] - t_size[1] - 10)
            cv2.rectangle(image, c1, (c2[0], np.float32(c3[1])), bbox_color, -1) #filled

            cv2.putText(image, bbox_mess, (np.float32(c1[0] + 5),np.float32(c1[1] - 5)), cv2.FONT_HERSHEY_SIMPLEX,
